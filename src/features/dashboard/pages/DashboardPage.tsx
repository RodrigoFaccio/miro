import PageContainer from "@/features/common/components/layout/PageContainer";
import {
  formatCurrency,
  formatPercentage,
} from "@/features/common/utils/formatters";
import DashboardStatWidget from "@/features/dashboard/components/DashboardStatWidget";
import { getDashboardData } from "@/features/dashboard/services";

import PeriodInput, {
  Period,
} from "@/features/common/components/dates/PeriodInput";
import DashboardChartPie from "@/features/dashboard/components/DashboardChartPie";
import DashboardAreaChart from "@/features/dashboard/components/DashboardDailyRevenueChart";
import DashboardRevenueChart from "@/features/dashboard/components/DashboardRevenueChart";
import { RevenueCard } from "@/features/dashboard/components/RevenueCard";
import { TotalRevenueCard } from "@/features/dashboard/components/TotalRevenueCard";
import { SalesData } from "@/features/dashboard/types";
import { InfoIcon } from "@chakra-ui/icons";
import {
  Box,
  Card,
  CircularProgress,
  Divider,
  Grid,
  GridItem,
  HStack,
  Heading,
  Icon,
  IconButton,
  Popover,
  PopoverArrow,
  PopoverBody,
  PopoverCloseButton,
  PopoverContent,
  PopoverHeader,
  PopoverTrigger,
  Portal,
  SimpleGrid,
  Stack,
  Tag,
  Text,
  useBreakpointValue,
  useToken,
} from "@chakra-ui/react";
import { useQuery } from "@tanstack/react-query";
import moment from "moment";
import { useMemo, useState } from "react";
import { BsCreditCard } from "react-icons/bs";
import { FaBarcode, FaBuildingCircleCheck, FaPix } from "react-icons/fa6";
import { IoIosPricetags, IoMdCash } from "react-icons/io";
import { IoCart } from "react-icons/io5";
import { RiRefund2Line } from "react-icons/ri";

const DashboardPage = () => {
  const [period, setPeriod] = useState(Period.LAST_7_DAYS);

  const [dates, setDates] = useState<{
    startDate: Date | null;
    endDate: Date | null;
  }>({
    startDate: moment().subtract(7, "days").toDate(),
    endDate: moment().toDate(),
  });

  const { data, isFetching } = useQuery({
    queryKey: ["dashboard", dates],
    queryFn: () => getDashboardData(dates),
  });

  const creditCardTotalInstallmentsProfit = useMemo(
    () =>
      data?.creditCard.installmentsProfit.reduce(
        (acc, item) => acc + item.profit,
        0,
      ),
    [data?.creditCard.installmentsProfit],
  );

  return (
    <PageContainer
      title={
        <Stack
          justifyContent="space-between"
          flexDirection={{
            base: "column",
            lg: "row",
          }}
        >
          <HStack spacing={4}>
            <Text>Dashboard</Text>
            {isFetching && (
              <CircularProgress
                isIndeterminate
                color="primary.500"
                size="20px"
              />
            )}
          </HStack>
          <Card variant="outlined" w={220} maxW="100%">
            <PeriodInput
              value={{
                period,
                startDate: dates.startDate,
                endDate: dates.endDate,
              }}
              onChange={({ period, startDate, endDate }) => {
                setPeriod(period);
                setDates({
                  startDate,
                  endDate,
                });
              }}
            />
          </Card>
        </Stack>
      }
      crumbs={[
        {
          href: "/",
          label: "Dashboard",
        },
      ]}
    >
      <Stack spacing={5}>
        <SimpleGrid
          columns={
            useBreakpointValue({
              base: 1,
              lg: 3,
            }) as number
          }
          spacing={5}
        >
          <DashboardStatWidget
            title="Total de vendas"
            stat={formatCurrency(data?.paidSalesAmount ?? 0)}
            icon={IoCart}
            color="primary.500"
          />
          <DashboardStatWidget
            title="Ticket médio"
            stat={formatCurrency(data?.averageTicket ?? 0)}
            icon={IoIosPricetags}
            color="primary.500"
          />
          <DashboardStatWidget
            title="Chargeback"
            subtitle={
              <>
                Porcentagem:{" "}
                <strong>
                  {formatPercentage(data?.chargeback.percentage || 0)}
                </strong>
              </>
            }
            stat={formatCurrency(data?.chargeback.amount || 0)}
            color={"orange.500"}
            tag={`${data?.chargeback.count || 0} vendas`}
            icon={RiRefund2Line}
          />
        </SimpleGrid>
        <SimpleGrid
          columns={
            useBreakpointValue({
              base: 1,
              md: 2,
              lg: 3,
            }) as number
          }
          spacing={5}
        >
          <DashboardStatWidget
            title="Vendas no Cartão de Crédito"
            subtitle={
              <HStack gap={1}>
                <Popover preventOverflow placement="bottom-start">
                  <PopoverTrigger>
                    <IconButton
                      aria-label="Detalhes"
                      icon={<InfoIcon />}
                      size="xs"
                      colorScheme="primary"
                      rounded="full"
                      variant="ghost"
                    />
                  </PopoverTrigger>
                  <Portal>
                    <PopoverContent w={250}>
                      <PopoverArrow />
                      <PopoverCloseButton />
                      <PopoverHeader>
                        <HStack>
                          <Icon as={BsCreditCard} />
                          <Heading size="xs">Taxas por Parcelamento</Heading>
                        </HStack>
                      </PopoverHeader>
                      <PopoverBody>
                        <Stack gap={1} fontFamily="monospace">
                          {data?.creditCard.installmentsProfit.map(
                            (installment) => (
                              <HStack
                                key={installment.installments}
                                justify="space-between"
                              >
                                <Text size="xs">
                                  {installment.installments}x
                                </Text>
                                <Divider
                                  borderColor="gray.500"
                                  borderStyle="dashed"
                                  flex={1}
                                />
                                <Tag
                                  colorScheme="primary"
                                  variant="subtle"
                                  size="sm"
                                >
                                  {formatCurrency(installment.profit)}
                                </Tag>
                              </HStack>
                            ),
                          )}
                          <HStack justify="space-between" mt={3}>
                            <Text size="xs" fontWeight={600}>
                              Total
                            </Text>
                            <Divider
                              borderColor="gray.500"
                              borderStyle="dashed"
                              flex={1}
                            />
                            <Tag
                              colorScheme="primary"
                              variant="subtle"
                              size="sm"
                            >
                              {formatCurrency(
                                creditCardTotalInstallmentsProfit || 0,
                              )}
                            </Tag>
                          </HStack>
                        </Stack>
                      </PopoverBody>
                    </PopoverContent>
                  </Portal>
                </Popover>
              </HStack>
            }
            stat={formatCurrency(data?.creditCard.paidAmount || 0)}
            color={"red.400"}
            tag={`${data?.creditCard.count || 0} vendas`}
            icon={BsCreditCard}
          />
          <DashboardStatWidget
            title="Vendas no Boleto"
            // subtitle={
            //   <>
            //     Volume total:{" "}
            //     <strong>{formatCurrency(data?.boleto.amount || 0)}</strong>
            //   </>
            // }
            stat={formatCurrency(data?.boleto.paidAmount || 0)}
            color={"blue.400"}
            tag={`${data?.boleto.count || 0} vendas`}
            icon={FaBarcode}
          />
          <DashboardStatWidget
            title="Vendas no Pix"
            // subtitle={
            //   <>
            //     Volume total:{" "}
            //     <strong>{formatCurrency(data?.pix.amount || 0)}</strong>
            //   </>
            // }
            stat={formatCurrency(data?.pix.paidAmount || 0)}
            color={"green.500"}
            tag={`${data?.pix.count || 0} vendas`}
            icon={FaPix}
          />
        </SimpleGrid>
        <Grid templateColumns="repeat(12, 1fr)" gap={5}>
          <Stack spacing={5} as={GridItem} colSpan={{ base: 12, lg: 6 }}>
            <Box w="full" h={450}>
              <DashboardAreaChart
                key={`${dates.startDate}-${dates.endDate}-revenue-by-period`}
                datetime={dates.startDate !== dates.endDate}
                dailyRevenue={data?.dailyRevenue ?? []}
                title="Faturamento por período"
                subtitle="Acompanhe o volume de vendas por período"
              />
            </Box>
          </Stack>

          <GridItem colSpan={{ base: 12, lg: 6 }}>
            <Stack w="full" spacing={5} h={450}>
              <DashboardRevenueChart
                creditCard={data?.creditCard ?? ({} as SalesData["creditCard"])}
                boleto={data?.boleto ?? ({} as SalesData["boleto"])}
                pix={data?.pix ?? ({} as SalesData["pix"])}
                title="Faturamento"
                subtitle="Acompanhe o faturamento da sua empresa"
              />
            </Stack>
          </GridItem>
        </Grid>

        <Grid templateColumns="repeat(12, 1fr)" gap={5}>
          <Stack as={GridItem} gap={5} colSpan={4}>
            <DashboardStatWidget
              title="Empresas Aprovadas"
              stat={String(data?.approvedCompanies || 0)}
              color={"green.500"}
              icon={FaBuildingCircleCheck}
            />
            <Box w="full" h={450}>
              <DashboardChartPie
                title="Vendas"
                data={[
                  {
                    name: "Cartão de crédito",
                    color: useToken("colors", "red.400"),
                    value: data?.creditCard?.revenue ?? 0,
                  },
                  {
                    name: "Boleto",
                    color: useToken("colors", "blue.400"),
                    value: data?.boleto?.revenue ?? 0,
                  },
                  {
                    name: "Pix",
                    color: useToken("colors", "green.500"),
                    value: data?.pix?.revenue ?? 0,
                  },
                ]}
              />
            </Box>
          </Stack>
          <Stack as={GridItem} colSpan={4} spacing={5}>
            <TotalRevenueCard
              title="Total"
              biling={data?.total.revenue || 0}
              profit={data?.total.profit || 0}
              rate={data?.total.fee || 0}
              summary={data?.total.summary || []}
            />
          </Stack>
          <Stack as={GridItem} colSpan={4} spacing={5}>
            {data?.summary.map((item) => (
              <RevenueCard
                biling={item.revenue || 0}
                profit={item.profit || 0}
                rate={item.fee || 0}
                title={item.label}
              />
            ))}
          </Stack>
        </Grid>
      </Stack>
    </PageContainer>
  );
};

export default DashboardPage;
